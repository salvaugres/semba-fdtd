integer function test_coaxial_line_paul_8_6_square() bind(C) result(error_cnt)    
    ...
    character(len=*), parameter :: square_excitation = PATH_TO_TEST_DATA//'mtln/coaxial_line_paul_8_6_0.1_square.exc'
...
integer function test_2_conductor_line_paul_9_6() bind(C) result(error_cnt)    
    ...
    block
        integer :: i
        cable%step_size = [(0.0006289308176100629, i = 1, 795)]
    end block
    ! cable%step_size = [20.0, 20.0, 20.0, 20.0, 20.0]

    parsed%time_step = 4.250797024442083e-12
    ! parsed%number_of_steps = 35
    parsed%number_of_steps = 8e-9/parsed%time_step
    ...
    probe_v_201%probe_type = PROBE_TYPE_VOLTAGE
