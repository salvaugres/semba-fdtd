...
contains
   ...
   subroutine expect_eq_mtln(err, ex, pr)
      ...
      !mtln
      if (.not. ex%mtln%cables == pr%mtln%cables) call testFails(err, 'Expected and read mtln "cables" do not match')
      if (.not. ex%mtln%probes == pr%mtln%probes) call testFails(err, 'Expected and read mtln "probes" do not match')
      if (.not. ex%mtln%networks == pr%mtln%networks) call testFails(err, 'Expected and read mtln "networks" do not match')
      if (err == 0) write(*,*) "Read and expected inputs are equal."      
