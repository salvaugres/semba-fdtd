...
contains
   ...
   subroutine InitWires_mtln(sgg,Ex,Ey,Ez,mtln_wir,thereAreMTLNbundles)         
      ...
      REAL (KIND=RKIND), intent(inout), target :: &
        ...
        do m=1,MTLN_WIR%NUMBER_OF_BUNDLES
            ...
            do n=1,number_of_segments                          
                i = MTLN_WIR%bundles(m)%SEGMENT_RELATIVE_POSITIONs(n)%position(1)
                j = MTLN_WIR%bundles(m)%SEGMENT_RELATIVE_POSITIONs(n)%position(2)
                k = MTLN_WIR%bundles(m)%SEGMENT_RELATIVE_POSITIONs(n)%position(3)    
                direction=MTLN_WIR%bundles(m)%SEGMENT_RELATIVE_POSITIONs(n)%direction
                select case (abs(direction))  
                case(1)                                
                    mtln_wir%bundles(m)%SEGMENT_RELATIVE_POSITIONs(n)%Efield_wire2main => Ex(I,J,K) 
                    mtln_wir%bundles(m)%SEGMENT_RELATIVE_POSITIONs(n)%Efield_main2wire => Ex(I,J,K) 
                case(2)     
                    mtln_wir%bundles(m)%SEGMENT_RELATIVE_POSITIONs(n)%Efield_wire2main => Ey(I,J,K)
                    mtln_wir%bundles(m)%SEGMENT_RELATIVE_POSITIONs(n)%Efield_main2wire => Ey(I,J,K)  
                case(3)     
                    mtln_wir%bundles(m)%SEGMENT_RELATIVE_POSITIONs(n)%Efield_wire2main => Ez(I,J,K)
                    mtln_wir%bundles(m)%SEGMENT_RELATIVE_POSITIONs(n)%Efield_main2wire => Ez(I,J,K)  
                end select
       ...
       real (kind=RKIND), dimension (:), intent(in) :: &
        ...
        do m=1,MTLN_WIR%NUMBER_OF_BUNDLES
            ...
            do n=1,number_of_segments                          
                ...
                k = mtln_wir%bundles(m)%SEGMENT_RELATIVE_POSITIONs(n)%position(3)
                direction=MTLN_WIR%bundles(m)%SEGMENT_RELATIVE_POSITIONs(n)%direction
                punt => mtln_wir%bundles(m)%SEGMENT_RELATIVE_POSITIONs(n)%Efield_wire2main
                select case (abs(direction))  
                case(1)
  ...
  !      call mtln_step(mtln_wir, currents, voltages) !!!AGB esto hay que tocarlo porque voltages y currents ya estan en punteros
        !
        do m=1,mtln_wir%NUMBER_OF_BUNDLES
            number_of_segments=1 !ubound(mtln_wir%bundles(m)%SEGMENT_RELATIVE_POSITIONS,1)
            do n=1,number_of_segments                          
                i =mtln_wir%bundles(m)%SEGMENT_RELATIVE_POSITIONs(n)%position(1)
                j =mtln_wir%bundles(m)%SEGMENT_RELATIVE_POSITIONs(n)%position(2)
                k =mtln_wir%bundles(m)%SEGMENT_RELATIVE_POSITIONs(n)%position(3)
